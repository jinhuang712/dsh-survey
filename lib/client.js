...
function useText() {
  ...
  const snapshot2 = readStore(face);
  const active = snapshot2 === null || snapshot2 === void 0 ? null : snapshot2.active;
...
function selectSurveyQuestion({ interactions }) {
  const found = (interactions || []).find(
    (i) => i && i.kind === "question" && Array.isArray(i.payload && i.payload.questions) && i.payload.questions.some((q) => q !== null && typeof q === "object" && q[SURVEY_MARKER] === true)
  );
  return found ?? null;
}
function PendingComposerHint() {
  const t = useText();
  const scrollToCard = () => {
    const card = document.querySelector(".mq-frame");
    if (card !== null && card !== void 0 && typeof card.scrollIntoView === "function") {
...
function apply(ctx) {
  ...
  if (locale !== void 0 && locale !== null && typeof locale.subscribe === "function") {
    setLocaleFace({
      subscribe: (listener2) => locale.subscribe(listener2),
