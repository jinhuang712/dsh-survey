window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// The locale is readable without a client, so the server snapshot is the
		// same one — omitting it makes useSyncExternalStore throw when rendered
		// outside the browser.
		const readStore = (face) => useStore(face.subscribe, face.getSnapshot, face.getSnapshot);
		...
		const BOOL_CHOICES = [{ value: "yes", key: "bool.yes" }, { value: "no", key: "bool.no" }];
				...
				return React.createElement("div", { className: "mq-grid-options" },
					["left", "right"].map((side) => {
						...
						return React.createElement("button", {
							...
							React.createElement(Mark, { index: side === "left" ? 1 : 2 }),
							React.createElement("span", { className: "mq-grid-opt-label" }, React.createElement(Md, { text: item.title ? item.title : t(side === "left" ? "compare.left" : "compare.right") }))
						);
					})
				);
			}
		...
		function apply(ctx) {
			injectCss();
			// Bind once: useSyncExternalStore wants a stable subscribe reference,
			// and the runtime's methods need their receiver.
