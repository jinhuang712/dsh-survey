window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * 14px checkbox; single-select options carry no number seat — the option
		 * label already carries its own A/B/① numbering, so a 1/2/3 seat would
		 * duplicate it (selection is shown by the row highlight). */
		function Mark(props) {
			if (props.check) {
		...
		function QuestionBody(props) {
			...
			if (isBool) {
				return React.createElement("div", { className: "mq-options" },
					React.createElement(OptionRow, { label: "是", selected: d.selected, multi: false, disabled, onChoose: props.onBool }),
					React.createElement(OptionRow, { label: "否", selected: d.selected, multi: false, disabled, onChoose: props.onBool })
				);
			}
			if (isCompare) {
