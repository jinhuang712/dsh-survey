window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Leading indicator, matching the composer look: single-select shows the
		 * option number seat, multi-select a 14px checkbox with a check icon. */
		function Mark(props) {
			if (props.check) {
		...
		function OptionRow(props) {
			...
			const display = parseRecommendedLabel(props.label);
