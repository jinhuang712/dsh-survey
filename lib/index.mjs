...
export function apply(ctx) {
  ...
  const tool = defineTool({
    ...
    async execute(args, exec) {
      ...
      // The owning session feeds the workspace-area pending reminder: the
      // sidebar badge knows which session waits and can jump to it.
      const sessionId = exec && exec.agent
        ? (exec.agent.session && exec.agent.session.id) || exec.agent.id
        : undefined;

      return new Promise((resolve, reject) => {
        const entry = { resolve: null, reject: null, ids, timer: undefined, sessionId, questionCount: ids.size, askedAt: Date.now() };
        let settled = false;
  ...
  disposers.push(
    webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          if (method === "GET" && path === "/api/dsh-survey/pending") {
            // The workspace reminder polls this: every survey still awaiting
            // the user, with the session that owns it (when the tool ran under
            // an agent). No question text crosses here — just enough to render
            // a badge and jump to the right session.
            const surveys = [...pending.entries()].map(([callId, entry]) => ({
              callId,
              sessionId: entry.sessionId,
              questionCount: entry.questionCount,
              askedAt: entry.askedAt
            }));
            surveys.sort((a, b) => a.askedAt - b.askedAt);
            sendJson(res, 200, { ok: true, surveys });
            return;
          }
